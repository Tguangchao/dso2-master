...
private:
	...
	// add by tgc
	 // 导入图片
    inline void LoadGutou() {
        std::cout << "Loading Kitti timestamps!" << std::endl;
        std::string timesFile = path + "/rgb.txt";
        LoadImages(timesFile, files, timestamps);
        std::cout << "Load total " << timestamps.size() << " data entries." << std::endl;
     ...
     * @param[in] vstrImageFilenames    彩色图片名称文件
     * @param[in] vTimestamps           记录时间戳
     */
    void LoadImages(const std::string &strFile, std::vector<std::string> &vstrImageFilenames, std::vector<double> &vTimestamps)
    {
        ...
        while(!f.eof())
        {
            ...
            if(!s.empty())
            {
                ...
                ss >> sRGB;
                vstrImageFilenames.push_back((path +  '/' + sRGB));
            }
        }
    }
	// add by tgc
